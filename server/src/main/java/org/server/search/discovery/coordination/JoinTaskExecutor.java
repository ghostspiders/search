...
public class JoinTaskExecutor implements ClusterStateTaskExecutor<JoinTaskExecutor.Task>  {
     ...
     */
    private final ShardsRoutingStrategy allocationService;
    ...
    public JoinTaskExecutor(Settings settings, ShardsRoutingStrategy allocationService, Logger logger) {
        this.allocationService = allocationService;
        this.logger = logger;
        minimumMasterNodesOnLocalNode = -1;
    }
    ...
    public ClusterTasksResult<Task> execute(ClusterState currentState, List<Task> joiningNodes) throws Exception {
        ...
        final Nodes currentNodes = currentState.nodes();
        ...
        Nodes.Builder nodesBuilder = Nodes.newNodesBuilder().putAll(newState.build().nodes());
        ...
        for (final Task joinTask : joiningNodes) {
            ...
            } else if (currentNodes.nodeExists(joinTask.node().getId())) {
                // 如果集群中已存在该节点，则记录日志
